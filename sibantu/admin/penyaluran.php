...
$sql = "
    SELECT p.*, 
            l.nama_pelapor, 
            l.lokasi, -- Tambahkan kolom lokasi dari laporan
            b.nama_bantuan 
    FROM penyaluran p
    LEFT JOIN laporan l ON p.id_laporan = l.id
    LEFT JOIN bantuan b ON p.id_bantuan = b.id
    ORDER BY p.created_at DESC
";
$penyaluran = $conn->query($sql);

if ($penyaluran === false) {
    die("Error Kueri SQL: " . $conn->error);
}
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php while ($p = $penyaluran->fetch_assoc()): ?>
                                <tr>
                                    <td>
                                        <?php echo date('d/m/Y', strtotime($p['tanggal_penyaluran'] ?? $p['created_at'])); ?>
                                    </td>
                                    ...
                                    <td><?php echo htmlspecialchars($p['petugas'] ?? '-'); ?></td>
